...
class AirQualityAgent:
    ...
    def predict_from_live_data(self):
        """
        Triggers a high-fidelity prediction cycle using pre-trained models.
        """
        from .modules.db import get_latest_context, get_recent_snapshots
        import numpy as np
        ...
        if not data:
            return {
                ...
                "confidence": 0,
                "explanation": "Agent: No live environment context available."
            }
            
        current_pm25 = float(data.get('pm25') or data.get('aqi') or 0)
        gas_index = float(data.get('gas') or data.get('gas_index') or 0)
        ...
        usage = float(data.get('usage_hours') or 0)
        
        # 2. RUN SMART PREDICTION
        # This uses the pre-trained .pkl model inside the agent
        predicted_pm25, model_conf = self.pollution_model.predict(
            current_pm25=current_pm25,
            hour_of_day=hour,
            gas_index=gas_index,
            fan_speed=0 # Assume monitoring mode for prediction baseline
        )
        
        # Sanitize output
        predicted_pm25 = max(0.0, round(float(predicted_pm25), 2))
        ...
        # Blend model-specific confidence with data-quality confidence
        final_confidence = (model_conf * 100 + conf_result["score"]) / 2.0
        
        # 4. GENERATE REASONING (LLM Engine)
        # We construct a mock 'output' for the reasoning engine to explain
        mock_output = {
            "status": {
                "current_air_quality_index": current_pm25,
                "predicted_next_hour_index": predicted_pm25,
                "spike_warning": self.pollution_model.is_spike_likely(current_pm25, predicted_pm25),
                "prediction_confidence": round(final_confidence / 100.0, 2)
            },
            "filter": {"health_percentage": 100, "status": "Good"}, # Placeholders
            "action": {"recommended_fan_speed": "OFF"} # Placeholder for explanation logic
        }
        
        explanation = self.reasoning.explain(mock_output)
        
        return {
          "predicted_aqi_1h": predicted_pm25,
