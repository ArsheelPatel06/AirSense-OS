...
import { useScrollScrub } from '../../shared/hooks/useScrollScrub';
...
const RevealSection: React.FC = () => {
    ...
    useScrollScrub({
        ...
        if (!particlesRef.current || !textRef.current) return;
        ...
        tl.fromTo(particles,
            ...
            {
                x: (i: number) => ((i % colCount) - colCount / 2) * gap,
                y: (i: number) => (Math.floor(i / colCount) - 5) * gap,
                opacity: (_i: number) => Math.random() * 0.4 + 0.6,
                scale: 1,
                duration: 1,
...
const OutcomeSection: React.FC = () => {
    ...
    useScrollScrub({
        ...
        tl.fromTo(massRef.current,
            { scale: 3, opacity: 0, filter: "blur(40px)" },
        ...
        tl.to(countObj.current, {
            ...
            onUpdate: () => {
                if (textRef.current) {
                    textRef.current.textContent = Math.floor(countObj.current.value).toString();
                }
            }
        }, 0);
    });
...
// FRAME 8: ENTRY
const EntrySection: React.FC<{ navigate: ReturnType<typeof useNavigate> }> = ({ navigate }) => {
    const sectionRef = useRef<HTMLDivElement>(null);
    ...
    return (
        <Section id="entry-section" className="section-entry bg-black text-white min-h-[60vh] relative overflow-hidden flex items-center justify-center">
            ...
            <div ref={contentRef} className="relative z-10 flex flex-col items-center max-w-2xl px-6 text-center">
                <div className="w-px h-16 bg-gradient-to-b from-transparent to-emerald-900/50 mb-8" />
...
export const Home: React.FC = () => {
    ...
    return (
        <div className="theme-landing bg-[var(--color-landing-bg)] text-[var(--color-landing-text)] selection:bg-[var(--color-landing-primary)] selection:text-white font-sans min-h-screen">
            <Navbar />
            <Hero />
            <ProblemSection />
            <RevealSection />
